...
Document.prototype._calculateVersion = function () {
  ...
  if (versionType === undefined) {
    throw new Errors.ValidationError("A version field with name: " + model._version + " cannot be found in the schema definition");
  }
  if (type.isDate(versionType)) {
    ...
    throw new Errors.ValidationError("Version columns can only be Date or Number");
  }
}
  ...
  var buildQuery = function () {
    if (self.__proto__._saved === false) {
      ...
      if (copy[model._pk] === undefined) {
        throw new Errors.ValidationError("The document was previously saved, but its primary key is undefined.");
      }
