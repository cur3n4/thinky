        ...
        r.table(self.getTableName()).insert(copies, _options).run().then(function(results) {
          if (results.errors === 0) {
            ...
            for(var i = 0; i < rejects.length; i++) {
              rejects[i](new Promise.OperationalError("An error occurred during the batch insert. Original results:\n"+JSON.stringify(results, null, 2)));
            }
          }
        }).error(reject);
      }
  ...
  var p = new Promise(function(resolve, reject) {
    if (ungroup) {
      ...
      util.tryCatch(function() {
        ...
        var newError = new Promise.OperationalError("The results could not be converted to instances of `"+self.getTableName()+"`\nDetailed error: "+error.message);
